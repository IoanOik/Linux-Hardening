...
# Ensure journald does not keep log files localy
if [[ ! -d '/etc/systemd/journald.conf.d' ]]; then
    mkdir /etc/systemd/journald.conf.d
fi
printf "%s\n" '[Journal]' 'Storage=none' 'ForwardToSyslog=yes' >/etc/systemd/journald.conf.d/custom.conf

# Ensure journald service is enabled and active
systemctl reload-or-restart systemd-journald.service &>/dev/null
